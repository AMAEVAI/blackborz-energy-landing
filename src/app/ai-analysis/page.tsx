...
const potentialConfig = {
  ...
  medium: { labelKey: 'priority.medium', color: 'text-amber-400', bg: 'bg-amber-500/10', bar: 'bg-amber-400' },
  high: { labelKey: 'priority.high', color: 'text-[#c8ff00]', bg: 'bg-[#c8ff00]/10', bar: 'bg-[#c8ff00]' },
  very_high: { labelKey: 'priority.high', color: 'text-emerald-400', bg: 'bg-emerald-500/10', bar: 'bg-emerald-400' },
};
    ...
    <div className="p-4 md:p-8">
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black text-white">{t('ai.title')}</h1>
            <p className="text-[#666] text-sm mt-1">{t('ai.subtitle')}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-3">
          <div className="text-xs text-[#555] font-medium uppercase tracking-wide mb-3">{t('ai.activeLeads')} · {activeLeads.length}</div>
                  ...
                  <div className="flex items-center gap-2 ml-3">
                    {result ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className={`text-xs px-2 py-0.5 rounded-full font-medium ${potentialConfig[result.potential].color} ${potentialConfig[result.potential].bg}`}>{t(potentialConfig[result.potential].labelKey)}</div>
                      </div>
                    ) : (
        ...
        <div className="sticky top-8">
          {selectedResult && selectedLead ? (
            <div className="bg-[#141414] border border-[#242424] rounded-2xl overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                {selectedResult.strengths.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2"><CheckCircle className="w-4 h-4 text-emerald-400" /><span className="text-sm font-semibold text-emerald-400">{t('ai.strengths')}</span></div>
                ...
                {selectedResult.risks.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2"><AlertTriangle className="w-4 h-4 text-amber-400" /><span className="text-sm font-semibold text-amber-400">{t('ai.risks')}</span></div>
                ...
                {selectedResult.nextSteps.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2"><ArrowRight className="w-4 h-4 text-blue-400" /><span className="text-sm font-semibold text-blue-400">{t('ai.nextSteps')}</span></div>
            ...
            <div className="bg-[#141414] border border-[#242424] rounded-2xl p-8 text-center">
              ...
              <p className="text-[#555] text-sm">{t('ai.select')}</p>
            </div>
          )}
        </div>
      </div>
