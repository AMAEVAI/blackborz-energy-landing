    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#c8ff00] hover:bg-[#b8ef00] text-black rounded-xl text-sm font-bold transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="text-xs text-[#555] font-medium uppercase tracking-wide mb-3">
            Активные лиды · {activeLeads.length}
          </div>
                  ...
                  <div className="flex items-center gap-2 ml-3">
                    {result ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className={`text-xs px-2 py-0.5 rounded-full font-medium ${potentialConfig[result.potential].color} ${potentialConfig[result.potential].bg}`}>
                          {potentialConfig[result.potential].label}
                        </div>
                      </div>
                    ) : (
                      <button
                        onClick={(e) => { e.stopPropagation(); analyzeOne(lead); }}
                        disabled={isLoading}
                        className="flex items-center gap-1.5 px-2.5 py-1 bg-[#1a1a1a] border border-[#2a2a2a] text-[#888] hover:text-[#c8ff00] hover:border-[#c8ff00]/30 rounded-lg text-xs transition-all disabled:opacity-50"
                      >
                        {isLoading ? (
                ...
                {result && (
                  <div className="mt-3 pt-3 border-t border-[#1e1e1e]">
                    <div className="h-1.5 bg-[#1a1a1a] rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${potentialConfig[result.potential].bar}`}
            ...
            <div className="bg-[#141414] border border-[#242424] rounded-2xl overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative w-20 h-20 flex-shrink-0">
                    <svg className="w-20 h-20 -rotate-90">
                      ...
                      <circle
                        ...
                        strokeLinecap="round"
                        strokeDasharray={`${(selectedResult.score / 100) * 201} 201`}
                      />
                    </svg>
                  ...
                  <div>
                    <div className={`text-base font-bold ${potentialConfig[selectedResult.potential].color}`}>
                      {potentialConfig[selectedResult.potential].label} потенциал
                    </div>
                ...
                {selectedResult.strengths.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-semibold text-emerald-400">Сильные стороны</span>
                    </div>
                    <ul className="space-y-1.5">
                      {selectedResult.strengths.map((s, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-[#aaa]">
                          <Star className="w-3 h-3 text-emerald-400/60 mt-0.5 flex-shrink-0" />
                          {s}
                        </li>
                      ))}
                    </ul>
                ...
                {selectedResult.risks.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-semibold text-amber-400">Риски</span>
                    </div>
                    <ul className="space-y-1.5">
                      {selectedResult.risks.map((r, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-[#aaa]">
                          <AlertTriangle className="w-3 h-3 text-amber-400/60 mt-0.5 flex-shrink-0" />
                          {r}
                        </li>
                      ))}
                    </ul>
                ...
                {selectedResult.nextSteps.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-semibold text-blue-400">Следующие шаги</span>
                    </div>
                    <ul className="space-y-1.5">
                      {selectedResult.nextSteps.map((s, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-[#aaa]">
                          <span className="text-xs text-blue-400/60 font-bold mt-0.5 flex-shrink-0">{i + 1}.</span>
                          {s}
                        </li>
                      ))}
                    </ul>
